...
from .features import load_frame, prepare_scoring_frame, prepare_training_frame
from .infer import build_forecast_rows
from .split import split_by_target_date


@dataclass
class CatBoostTrainedOutputs:
...
CatBoostTuningEntry = tuple[int, float, int, int]  # depth, lr, l2_leaf_reg, border_count
...
def _build_catboost_frame(
    ...
    """Build a feature DataFrame suitable for catboost.Pool (no one-hot encoding).
    
    Drops target/meta columns but keeps raw categorical string columns so that
    CatBoost can handle them natively via cat_features.
    """
    base_columns = [
        column
        for column in frame.columns
        if column not in set([m for m in META_COLUMNS if m not in CATEGORICAL_FEATURE_COLUMNS] + [TARGET_COLUMN, COUNT_TARGET_COLUMN, "baseline_top_factors", "baseline_confidence"])
        and column not in {"location", "cluster_id", "police_station", "service_date", "target_next_shift_date", "target_next_shift"}
        and not column.startswith("target_next_shift_")
    ]

    # All non-categorical features must be numeric float
    cat_set = set(CATEGORICAL_FEATURE_COLUMNS)
    feature_frame = frame[base_columns].copy()
    for col in feature_frame.columns:
        if col not in cat_set:
            feature_frame[col] = pd.to_numeric(feature_frame[col], errors="coerce").fillna(0.0).astype(float)

    if reference_columns is not None:
        for col in reference_columns:
            if col not in feature_frame.columns:
                feature_frame[col] = 0.0 if col not in cat_set else ""
        feature_frame = feature_frame[reference_columns]

    final_cols = list(feature_frame.columns)
    return feature_frame, final_cols
...
def _apply_dormancy_guard(
    ...
    """Identical dormancy guard logic from train.py."""
...
def _train_catboost_models(
    ...
    cls_params_override: dict[str, Any],
    reg_params_override: dict[str, Any],
    cat_features: list[str],
) -> CatBoostTrainedOutputs:
    """Train CatBoost classifier (Logloss) and regressor (RMSE) using native API."""

    train_pool_cls = catboost.Pool(
        data=train_x,
        label=train_y,
        cat_features=cat_features,
    )
    predict_pool_cls = catboost.Pool(
        data=predict_x,
        cat_features=cat_features,
    )
    ...
    cls_params.update(cls_params_override)

    classifier = catboost.train(
        pool=train_pool_cls,
        params=cls_params,
        eval_set=predict_pool_cls,
    )
    probabilities_raw = classifier.predict(predict_pool_cls, prediction_type="Probability")
    # CatBoost returns (n, 2) for binary classification — extract positive class
    if probabilities_raw.ndim == 2 and probabilities_raw.shape[1] >= 2:
        probabilities = probabilities_raw[:, 1].ravel()
    else:
        probabilities = np.asarray(probabilities_raw).ravel()

    train_pool_reg = catboost.Pool(
        data=train_x,
        label=train_counts,
        cat_features=cat_features,
    )
    predict_pool_reg = catboost.Pool(
        data=predict_x,
        cat_features=cat_features,
    )
    ...
    reg_params.update(reg_params_override)

    regressor = catboost.train(
        pool=train_pool_reg,
        params=reg_params,
        eval_set=predict_pool_reg,
    )
    predicted_counts = np.maximum(0.0, np.asarray(regressor.predict(predict_pool_reg)).ravel())

    return CatBoostTrainedOutputs(
        probabilities=np.asarray(probabilities).ravel(),
        predicted_counts=np.asarray(predicted_counts).ravel(),
    )


def _build_tuning_entry(
    depth: int,
    learning_rate: float,
    l2_leaf_reg: int,
    border_count: int,
...
def _evaluate_baseline(frame: pd.DataFrame) -> dict[str, float]:
    """Evaluate the heuristic baseline on a dataframe."""
...
def _catboost_candidate_trials(
    split: Any,
    train_x: pd.DataFrame,
    validation_x: pd.DataFrame,
    cat_features: list[str],
) -> tuple[list[CatBoostTrial], dict[str, float], dict[str, float]]:
    ...
    # Build combined train+val for final testing
    ...
    combined_x, combined_columns = _build_catboost_frame(combined_prepared)
    ...
    test_x, _ = _build_catboost_frame(test_prepared, combined_columns)

    # Build reduced tuning grid (resource-aware)
    depths = [6, 8]
    learning_rates = [0.05, 0.07]
    l2_leaf_regs = [3, 5]
    border_counts = [64, 128]

    trials: list[CatBoostTrial] = []
    index = 0

    for depth, lr, l2r, bc in itertools.product(depths, learning_rates, l2_leaf_regs, border_counts):
        index += 1
        override = _build_tuning_entry(depth, lr, l2r, bc)

        validation_outputs = _train_catboost_models(
            train_x,
            train_labels,
            train_counts,
            validation_x,
        ...
        # Test evaluation with combined train+val training
        test_outputs = _train_catboost_models(
            combined_x,
            combined_labels,
            combined_counts,
            test_x,
        ...
        label = f"depth{depth}_lr{lr}_l2{l2r}_bc{bc}"
        trials.append(
            CatBoostTrial(
                family_key="catboost",
                family="CatBoost",
                model_name="catboost_hotspot_classifier_v2",
                variant_label=label,
                tuning_index=index,
                params_override={
                    "depth": depth,
...
def _select_champion(trials: list[CatBoostTrial]) -> CatBoostTrial:
    return max(
        trials,
        key=lambda trial: (
...
def _build_scoring_forecast(
    ...
    combined_x, feature_columns = _build_catboost_frame(combined_prepared)
    scoring_x, _ = _build_catboost_frame(scoring_prepared, feature_columns)

    scoring_outputs = _train_catboost_models(
        combined_x,
...
def _top_error_rows(
    frame: pd.DataFrame, threshold: float, error_type: str, limit: int = 12
) -> list[dict[str, Any]]:
...
def _station_error_summary(frame: pd.DataFrame, threshold: float) -> list[dict[str, Any]]:
    ...
    return sorted(
        summaries,
        key=lambda item: (item["false_negative"], item["false_positive"], item["actual_positive_rows"]),
        reverse=True,
    )[:12]


def _build_operating_policy(
    validation_frame: pd.DataFrame, test_frame: pd.DataFrame
) -> dict[str, Any]:
    def summarize_scope(
        scope_name: str,
        validation_scope: pd.DataFrame,
        test_scope: pd.DataFrame,
        beta: float,
    ) -> dict[str, Any]:
...
def _build_champion_test_frame(
    split: Any,
    champion: CatBoostTrial,
    cat_features: list[str],
) -> pd.DataFrame:
    ...
    test_prepared = prepare_training_frame(split.test)
    combined_x, feature_columns = _build_catboost_frame(combined_prepared)
    test_x, _ = _build_catboost_frame(test_prepared, feature_columns)
    final_outputs = _train_catboost_models(
        combined_x,
        combined_prepared[TARGET_COLUMN].to_numpy(dtype=int),
        combined_prepared[COUNT_TARGET_COLUMN].to_numpy(dtype=float),
        test_x,
...
def _build_champion_validation_frame(
    split: Any,
    champion: CatBoostTrial,
    cat_features: list[str],
) -> pd.DataFrame:
    ...
    train_x, feature_columns = _build_catboost_frame(train_prepared)
    validation_x, _ = _build_catboost_frame(validation_prepared, feature_columns)
    outputs = _train_catboost_models(
        train_x,
...
def build_artifacts(processed_dir: Path) -> dict[str, Any]:
    ...
    cat_features = CATEGORICAL_FEATURE_COLUMNS  # ['shift', 'weekday', 'severity_band']

    train_x, feature_columns = _build_catboost_frame(split.train)
    validation_x, _ = _build_catboost_frame(split.validation, feature_columns)
    trials, baseline_validation, baseline_test = _catboost_candidate_trials(
        split, train_x, validation_x, cat_features
    )
    champion = _select_champion(trials)
    forecast_rows = _build_scoring_forecast(champion, split, scoring_frame, cat_features)
    validation_frame = _build_champion_validation_frame(split, champion, cat_features)
    test_frame = _build_champion_test_frame(split, champion, cat_features)
        ...
        "candidates": [
            {
                ...
                "validation_average_precision_gain": round(
                    trial.validation_calibrated_metrics["average_precision"]
                    - trial.validation_raw_metrics["average_precision"],
                    4,
                ),
                "validation_brier_gain": round(
                    trial.validation_raw_metrics["brier_score"]
                    - trial.validation_calibrated_metrics["brier_score"],
                    4,
                ),
            }
            for trial in trials
    ...
    validation_summary = {
        ...
        "notes": [
            "CatBoost hyperparameter tuning was run over depth, learning_rate, l2_leaf_reg, and border_count grid on the validation window.",
            ...
            "Categorical features (shift, weekday, severity_band) are handled natively by CatBoost via cat_features in Pool constructor.",
        ],
    }
    ...
    benchmark_summary = {
        ...
        "metrics": [
            _metric_entry(
                ...
                champion.test_calibrated_metrics["average_precision"],
                baseline_test["average_precision"], "higher",
            ),
            _metric_entry(
                "precision_at_risk_55", "Precision @ Risk 55",
                champion.test_calibrated_metrics["precision_at_risk_55"],
                baseline_test["precision_at_risk_55"], "higher",
            ),
            _metric_entry(
                "recall_at_risk_55", "Recall @ Risk 55",
                champion.test_calibrated_metrics["recall_at_risk_55"],
                baseline_test["recall_at_risk_55"], "higher",
            ),
            _metric_entry(
                "precision_at_top_30", "Precision @ Top 30",
                champion.test_calibrated_metrics["precision_at_top_30"],
                baseline_test["precision_at_top_30"], "higher",
            ),
            _metric_entry(
                "recall_at_top_30", "Recall @ Top 30",
                champion.test_calibrated_metrics["recall_at_top_30"],
                baseline_test["recall_at_top_30"], "higher",
            ),
            _metric_entry(
                "mae_next_shift_records", "Next-shift MAE",
                champion.test_calibrated_metrics["mae_next_shift_records"],
                baseline_test["mae_next_shift_records"], "lower",
            ),
            _metric_entry(
                "log_loss", "Log Loss",
                champion.test_calibrated_metrics["log_loss"],
                baseline_test["log_loss"], "lower",
            ),
            _metric_entry(
                "brier_score", "Brier Score",
                champion.test_calibrated_metrics["brier_score"],
                baseline_test["brier_score"], "lower",
            ),
        ],
        "summary_note": (
            f"{champion.model_name} (CatBoost) was selected after tuning and calibration on the validation window, "
    ...
    error_analysis_report = {
        ...
        "test_reliability": {
            "raw": reliability_bins(
                test_frame[TARGET_COLUMN].to_numpy(dtype=int),
                test_frame["raw_probability"].to_numpy(dtype=float),
            ),
            "calibrated": reliability_bins(
                test_frame[TARGET_COLUMN].to_numpy(dtype=int),
                test_frame["predicted_probability"].to_numpy(dtype=float),
            ),
        },
...
def main() -> None:
    for processed_dir in PROCESSED_DIRS:
        ...
        print(f"CatBoost ML artifacts written to: {processed_dir.resolve()}")
